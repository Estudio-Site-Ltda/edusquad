...
const outDir = path.join(root, 'squads', slug, 'output', runId, 'slides-capitulo-02');
const imagesDir = path.join(outDir, 'images');
const iconsDir = path.join(outDir, 'icons');
const fontsDir = path.join(outDir, 'fonts');
const htmlPath = path.join(outDir, 'slide.html');

const heroImage = path.join(root, 'assets', 'images', slug, 'capitulo-02-planejamento-metas-negocio-lucid-realism-leonardo-1fd9fe6b.jpg');
const caseImage = path.join(root, 'assets', 'images', slug, 'capitulo-02-atelie-dona-clara-metas-kits-lucid-realism-leonardo-bcb82075.jpg');
...
function setupAssets() {
  ...
  ensureDir(iconsDir);
  ensureDir(fontsDir);
  copyFile(heroImage, path.join(imagesDir, 'capitulo-02-planejamento-metas.jpg'));
  copyFile(caseImage, path.join(imagesDir, 'capitulo-02-atelie-metas.jpg'));
...
<title>Capítulo 2 | Planejamento simples e metas possíveis</title>
...
.callout{display:flex;gap:.75rem;padding:1rem;border:1px solid rgba(29,138,156,.32);border-radius:var(--radius);background:var(--primary-50);color:var(--neutral-600);font-size:.88rem}.callout strong{color:var(--neutral-900)}.source-note{margin-top:1rem;color:var(--neutral-500);font-size:.72rem;line-height:1.45}
...
${slide('slide-1', 'Slide 1: abertura', 'cover active', `
  <div class="inner">
    ...
    <aside class="hero-panel reveal reveal-scale delay-2">
      <figure><img src="images/capitulo-02-planejamento-metas.jpg" alt="Empreendedora planejando metas de negócio em mesa com caderno e computador."></figure>
      <div class="metadata">
        <div class="meta-row"><span class="icon-shell">${icon('target')}</span><div><strong>Objetivo</strong><span>Criar metas realistas para os próximos 90 dias.</span></div></div>
...
${slide('slide-2', 'Slide 2: mapa do capítulo', 'summary tint', `
  <div class="inner">
    <p class="slide-context reveal reveal-up delay-0"><strong>Capítulo 2</strong><span>·</span><span>Mapa</span></p>
    <header class="summary-header reveal reveal-up delay-1">
      <span class="tag">O que você vai praticar</span>
      <h2 class="section-title">Planejar é escolher <span class="highlight">poucas prioridades executáveis</span></h2>
      <p class="lead" style="margin:.75rem auto 0">Você não precisa prever tudo. Precisa decidir o próximo ciclo, acompanhar o avanço e corrigir antes de perder tempo ou dinheiro.</p>
    </header>
    <div class="summary-grid stagger-children delay-2">
      <article class="card"><span class="icon-shell">${icon('target')}</span><h3>Meta clara</h3><p>Transformar desejo em número, prazo e resultado observável.</p></article>
      <article class="card"><span class="icon-shell">${icon('clipboard-text')}</span><h3>Plano de ação</h3><p>Definir o que será feito, por quem, até quando e com qual recurso.</p></article>
      <article class="card"><span class="icon-shell">${icon('calendar-check')}</span><h3>Revisão semanal</h3><p>Acompanhar ações e ajustar rota sem esperar o fim do mês.</p></article>
    </div>
  </div>`)}
${slide('slide-3', 'Slide 3: desejo versus meta', 'concept', `
  <div class="inner">
    <figure class="photo-card reveal reveal-left delay-1">
      <img src="images/capitulo-02-planejamento-metas.jpg" alt="Empreendedora organizando metas e ações em um pequeno negócio.">
      <figcaption>Uma meta boa cabe no papel, no calendário e no caixa.</figcaption>
    </figure>
    <div class="concept-copy">
      <p class="slide-context reveal reveal-up delay-0"><strong>Capítulo 2</strong><span>·</span><span>Conceito</span></p>
      <p class="eyebrow reveal reveal-up delay-1">Ideia central</p>
      <h2 class="section-title reveal reveal-up delay-2">Desejo mostra direção. <span class="highlight">Meta orienta execução.</span></h2>
      <div class="feature-list stagger-children delay-3">
        <article class="feature"><span class="icon-shell">${icon('warning')}</span><div><h3>Desejo</h3><p>“Quero vender mais.” É legítimo, mas ainda não orienta o que fazer amanhã.</p></div></article>
        <article class="feature"><span class="icon-shell">${icon('check-circle')}</span><div><h3>Meta</h3><p>“Vender 20 kits em 30 dias oferecendo renovação para clientes antigos.”</p></div></article>
        <article class="feature"><span class="icon-shell">${icon('chart-bar')}</span><div><h3>Indicador</h3><p>Mostra se a ação está aproximando você do resultado combinado.</p></div></article>
      </div>
    </div>
  </div>`)}
${slide('slide-4', 'Slide 4: anatomia da meta', '', `
  <div class="inner">
    <p class="slide-context reveal reveal-up delay-0"><strong>Capítulo 2</strong><span>·</span><span>Meta bem formada</span></p>
    <header style="max-width:820px;margin-bottom:1.4rem">
      <p class="eyebrow reveal reveal-up delay-1">Design instrucional: exemplo trabalhado</p>
      <h2 class="section-title reveal reveal-up delay-2">Uma meta útil tem <span class="highlight">quatro partes</span></h2>
    </header>
    <div class="grid-4 stagger-children delay-3">
      <article class="card"><span class="icon-shell">${icon('target')}</span><h3>Resultado</h3><p>O que você quer mudar: vendas, margem, recorrência, atraso, estoque ou satisfação.</p></article>
      <article class="card"><span class="icon-shell">${icon('chart-bar')}</span><h3>Número</h3><p>Quanto precisa mudar. Sem número, você depende de sensação.</p></article>
      <article class="card"><span class="icon-shell">${icon('calendar-check')}</span><h3>Prazo</h3><p>Até quando será medido. Para pequenos negócios, 30 a 90 dias costuma funcionar melhor.</p></article>
      <article class="card"><span class="icon-shell">${icon('users-three')}</span><h3>Dono</h3><p>Quem acompanha a execução e cobra a revisão, mesmo que seja você.</p></article>
    </div>
    <div class="callout reveal reveal-up delay-4" style="margin-top:1.1rem">${icon('lightbulb')} <p><strong>Modelo:</strong> Em 90 dias, aumentar em 15% a venda de serviços recorrentes, com revisão toda sexta-feira.</p></div>
  </div>`)}
${slide('slide-5', 'Slide 5: regra brasileira e segurança', 'dark', `
  <div class="inner" style="display:grid;grid-template-columns:1fr 1fr;gap:3rem;align-items:center">
    <div>
      <p class="slide-context reveal reveal-up delay-0"><strong>Capítulo 2</strong><span>·</span><span>Brasil</span></p>
      <p class="eyebrow reveal reveal-up delay-1">Meta também precisa caber nas regras</p>
      <h2 class="section-title reveal reveal-up delay-2">Antes de crescer, confira <span class="highlight">o limite e as obrigações</span></h2>
      <p class="lead reveal reveal-up delay-3">Se você é MEI, suas metas de venda devem considerar o limite anual, pagamento mensal do DAS, declaração anual e emissão de nota quando vender para pessoa jurídica.</p>
    </div>
    <div class="grid-2 stagger-children delay-2">
      <article class="card"><span class="icon-shell">${icon('money')}</span><h3>R$ 81 mil/ano</h3><p>Limite de receita bruta anual do MEI, proporcional no ano de abertura.</p></article>
      <article class="card"><span class="icon-shell">${icon('calendar-check')}</span><h3>DASN-SIMEI</h3><p>Declaração anual deve ser enviada até 31 de maio, mesmo sem faturamento.</p></article>
      <article class="card"><span class="icon-shell">${icon('clipboard-text')}</span><h3>Nota fiscal</h3><p>Obrigatória na venda ou serviço para pessoa jurídica, salvo regra específica.</p></article>
      <article class="card"><span class="icon-shell">${icon('notebook')}</span><h3>Registro mensal</h3><p>Relatório e documentos ajudam você a acompanhar limite, caixa e comprovações.</p></article>
    </div>
    <p class="source-note reveal reveal-up delay-4">Fontes verificadas em 01/06/2026: Gov.br/Portal do Empreendedor, Receita Federal/Simples Nacional e Portal Nacional NFS-e.</p>
  </div>`)}
${slide('slide-6', 'Slide 6: comparação interativa de metas', 'tabs-slide', `
  <div class="inner narrow">
    <p class="slide-context reveal reveal-up delay-0"><strong>Capítulo 2</strong><span>·</span><span>Interativo</span></p>
    <header style="margin-bottom:1.2rem">
      <p class="eyebrow reveal reveal-up delay-1">Clique nas abas</p>
      <h2 class="section-title reveal reveal-up delay-2">Compare três formas de escrever <span class="highlight">a mesma intenção</span></h2>
    </header>
    <div class="tabs reveal reveal-scale delay-3">
      <div class="tablist" role="tablist" aria-label="Qualidade da meta">
        <button class="tab" id="tab-1" role="tab" aria-selected="true" aria-controls="panel-1">Fraca</button>
        <button class="tab" id="tab-2" role="tab" aria-selected="false" aria-controls="panel-2" tabindex="-1">Melhor</button>
        <button class="tab" id="tab-3" role="tab" aria-selected="false" aria-controls="panel-3" tabindex="-1">Aplicável</button>
      </div>
      <div class="tabpanel" id="panel-1" role="tabpanel" aria-labelledby="tab-1"><div><h3>“Quero vender mais.”</h3><p>Mostra vontade, mas não informa quanto, até quando, para quem ou com qual ação.</p></div><aside class="side-note"><p class="eyebrow">Risco</p><strong>Você não sabe se avançou.</strong><p>Sem número e prazo, qualquer melhora parece suficiente.</p></aside></div>
      <div class="tabpanel" id="panel-2" role="tabpanel" aria-labelledby="tab-2" hidden><div><h3>“Quero vender 20 kits.”</h3><p>Já tem número, mas ainda falta prazo, público, responsável e rotina de acompanhamento.</p></div><aside class="side-note"><p class="eyebrow">Melhorou</p><strong>Agora existe alvo.</strong><p>O próximo passo é ligar o alvo às ações.</p></aside></div>
      <div class="tabpanel" id="panel-3" role="tabpanel" aria-labelledby="tab-3" hidden><div><h3>“Vender 20 kits em 30 dias.”</h3><p>Oferta feita para clientes dos últimos seis meses, com revisão semanal das conversas e vendas fechadas.</p></div><aside class="side-note"><p class="eyebrow">Use assim</p><strong>Agora cabe no calendário.</strong><p>A meta orienta ação, acompanhamento e ajuste.</p></aside></div>
    </div>
  </div>`)}
${slide('slide-7', 'Slide 7: método em cinco passos', 'tint', `
  <div class="inner">
    <p class="slide-context reveal reveal-up delay-0"><strong>Capítulo 2</strong><span>·</span><span>Método</span></p>
    <header style="max-width:780px">
      <p class="eyebrow reveal reveal-up delay-1">Plano de 90 dias</p>
      <h2 class="section-title reveal reveal-up delay-2">Cinco passos para sair da ideia e chegar na <span class="highlight">execução semanal</span></h2>
    </header>
    <div class="flow stagger-children delay-3">
      <article class="step"><span>1</span><h3>Escolha</h3><p>Defina um objetivo principal para os próximos 90 dias.</p></article>
      <article class="step"><span>2</span><h3>Transforme</h3><p>Adicione número, prazo e resultado esperado.</p></article>
      <article class="step"><span>3</span><h3>Liste</h3><p>Escolha até cinco ações que influenciam esse número.</p></article>
      <article class="step"><span>4</span><h3>Distribua</h3><p>Defina responsável, custo e data para cada ação.</p></article>
      <article class="step"><span>5</span><h3>Revise</h3><p>Compare ação feita com resultado obtido toda semana.</p></article>
    </div>
  </div>`)}
${slide('slide-8', 'Slide 8: flip cards de conceitos', '', `
  <div class="inner">
    <p class="slide-context reveal reveal-up delay-0"><strong>Capítulo 2</strong><span>·</span><span>Fixação</span></p>
    <header style="margin-bottom:1.35rem;max-width:780px">
      <p class="eyebrow reveal reveal-up delay-1">Clique para revelar</p>
      <h2 class="section-title reveal reveal-up delay-2">Quatro termos para planejar <span class="highlight">sem complicar</span></h2>
    </header>
    <div class="flip-grid reveal reveal-scale delay-3">
      <div class="flip-card" onclick="doFlip(this)" tabindex="0" role="button" aria-label="Virar card Meta"><div class="flip-inner"><div class="flip-front">${icon('target','icon-lg')}<h3>Meta</h3><span class="flip-hint">Clique para revelar</span></div><div class="flip-back"><h3>Meta</h3><p>É um resultado desejado escrito com número, prazo e condição de acompanhamento.</p></div></div></div>
      <div class="flip-card" onclick="doFlip(this)" tabindex="0" role="button" aria-label="Virar card Indicador"><div class="flip-inner"><div class="flip-front">${icon('chart-bar','icon-lg')}<h3>Indicador</h3><span class="flip-hint">Clique para revelar</span></div><div class="flip-back"><h3>Indicador</h3><p>É o número que mostra se você está se aproximando ou se precisa corrigir a rota.</p></div></div></div>
      <div class="flip-card" onclick="doFlip(this)" tabindex="0" role="button" aria-label="Virar card Plano de ação"><div class="flip-inner"><div class="flip-front">${icon('clipboard-text','icon-lg')}<h3>Plano de ação</h3><span class="flip-hint">Clique para revelar</span></div><div class="flip-back"><h3>Plano de ação</h3><p>É a lista curta de ações, responsáveis, prazos, custos e situação de andamento.</p></div></div></div>
      <div class="flip-card" onclick="doFlip(this)" tabindex="0" role="button" aria-label="Virar card Revisão"><div class="flip-inner"><div class="flip-front">${icon('calendar-check','icon-lg')}<h3>Revisão</h3><span class="flip-hint">Clique para revelar</span></div><div class="flip-back"><h3>Revisão</h3><p>É o momento fixo em que você aprende com o que aconteceu e decide o ajuste seguinte.</p></div></div></div>
    </div>
  </div>`)}
${slide('slide-9', 'Slide 9: estudo de caso Ateliê Dona Clara', 'concept', `
  <div class="inner">
    <figure class="photo-card reveal reveal-left delay-1">
      <img src="images/capitulo-02-atelie-metas.jpg" alt="Empreendedora em ateliê artesanal com produtos organizados e caderno de planejamento.">
      <figcaption>Ateliê Dona Clara: muitos pedidos pequenos, pouco lucro e nenhuma meta clara de mix de produtos.</figcaption>
    </figure>
    <div>
      <p class="slide-context reveal reveal-up delay-0"><strong>Capítulo 2</strong><span>·</span><span>Estudo de caso</span></p>
      <p class="eyebrow reveal reveal-up delay-1">Situação realista</p>
      <h2 class="section-title reveal reveal-up delay-2">O ateliê vendia, mas <span class="highlight">não direcionava esforço para margem</span></h2>
      <div class="feature-list stagger-children delay-3">
        <article class="feature"><span class="icon-shell">${icon('warning')}</span><div><h3>Problema</h3><p>Muitos pedidos pequenos consumiam tempo e deixavam pouco resultado no fim do mês.</p></div></article>
        <article class="feature"><span class="icon-shell">${icon('target')}</span><div><h3>Meta</h3><p>Promover kits de maior valor por 90 dias, com revisão semanal de vendas.</p></div></article>
        <article class="feature"><span class="icon-shell">${icon('check-circle')}</span><div><h3>Resultado</h3><p>O faturamento não explodiu, mas o lucro melhorou porque a margem dos kits era maior.</p></div></article>
      </div>
    </div>
  </div>`)}
${slide('slide-10', 'Slide 10: análise guiada em accordion', 'tint', `
  <div class="inner narrow">
    <p class="slide-context reveal reveal-up delay-0"><strong>Capítulo 2</strong><span>·</span><span>Análise guiada</span></p>
    <header style="margin-bottom:1.35rem">
      <p class="eyebrow reveal reveal-up delay-1">Abra cada parte</p>
      <h2 class="section-title reveal reveal-up delay-2">Veja como uma meta muda <span class="highlight">a decisão comercial</span></h2>
    </header>
    <div class="accord-list reveal reveal-scale delay-3">
      <div class="accord-item"><button class="accord-btn" aria-expanded="false" onclick="toggleAcc(this)">1. Por que “vender mais” era insuficiente?<svg class="accord-caret" viewBox="0 0 16 16" fill="none"><path d="M4 6l4 4 4-4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg></button><div class="accord-body"><div class="accord-content"><p>Porque a frase não separava volume, margem, produto prioritário nem prazo. A equipe trabalhava muito, mas sem direção clara.</p></div></div></div>
      <div class="accord-item"><button class="accord-btn" aria-expanded="false" onclick="toggleAcc(this)">2. Qual decisão a meta permitiu tomar?<svg class="accord-caret" viewBox="0 0 16 16" fill="none"><path d="M4 6l4 4 4-4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg></button><div class="accord-body"><div class="accord-content"><p>Priorizar kits de maior valor, ajustar oferta e medir semanalmente quantas conversas viraram venda.</p></div></div></div>
      <div class="accord-item"><button class="accord-btn" aria-expanded="false" onclick="toggleAcc(this)">3. O que você deve copiar desse caso?<svg class="accord-caret" viewBox="0 0 16 16" fill="none"><path d="M4 6l4 4 4-4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg></button><div class="accord-body"><div class="accord-content"><p>Escolha uma meta que melhore resultado, não apenas movimento. Mais venda só ajuda quando também respeita margem, caixa e capacidade.</p></div></div></div>
    </div>
  </div>`)}
${slide('slide-11', 'Slide 11: checklist interativo', 'dark', `
  <div class="inner mission-layout">
    <div>
      <p class="slide-context reveal reveal-up delay-0"><strong>Capítulo 2</strong><span>·</span><span>Missão prática</span></p>
      <p class="eyebrow reveal reveal-up delay-1">Marque conforme concluir</p>
      <h2 class="section-title reveal reveal-up delay-2">Monte sua primeira <span class="highlight">meta de 90 dias</span></h2>
      <p class="lead reveal reveal-up delay-3">A meta precisa caber no seu calendário, respeitar seu caixa e orientar ações reais da próxima semana.</p>
    </div>
    <div class="ms-panel reveal reveal-scale delay-2">
      <p class="ms-title">Sua missão de planejamento</p>
      <p class="ms-sub">Clique para marcar cada etapa concluída.</p>
      <div class="ms-list">
        <div class="ms-item" onclick="msTgl(this)" role="checkbox" aria-checked="false" tabindex="0"><div class="ms-check"></div>Escrever um objetivo principal de 90 dias</div>
        <div class="ms-item" onclick="msTgl(this)" role="checkbox" aria-checked="false" tabindex="0"><div class="ms-check"></div>Adicionar número, prazo e responsável</div>
        <div class="ms-item" onclick="msTgl(this)" role="checkbox" aria-checked="false" tabindex="0"><div class="ms-check"></div>Listar até 5 ações semanais</div>
        <div class="ms-item" onclick="msTgl(this)" role="checkbox" aria-checked="false" tabindex="0"><div class="ms-check"></div>Agendar a primeira revisão</div>
      </div>
      <div class="xp-label"><span>Progresso</span><span class="xp-pct">0%</span></div>
      <div class="xp-track"><div class="xp-bar"></div></div>
      <div class="ms-badge">Meta pronta para teste.</div>
    </div>
  </div>`)}
${slide('slide-12', 'Slide 12: mini quiz', '', `
  <div class="inner narrow">
    <p class="slide-context reveal reveal-up delay-0"><strong>Capítulo 2</strong><span>·</span><span>Checagem rápida</span></p>
    <header style="margin-bottom:1rem">
      <p class="eyebrow reveal reveal-up delay-1">Teste sua meta</p>
      <h2 class="section-title reveal reveal-up delay-2">Qual alternativa está <span class="highlight">mais pronta para execução</span>?</h2>
    </header>
    <div class="quiz-options reveal reveal-scale delay-3" data-quiz>
      <button class="quiz-btn" type="button" data-answer="wrong"><span class="icon-shell">${icon('warning')}</span><div><strong>A</strong><br><span>Quero crescer bastante nos próximos meses.</span></div></button>
      <button class="quiz-btn" type="button" data-answer="wrong"><span class="icon-shell">${icon('clock')}</span><div><strong>B</strong><br><span>Quero vender mais kits para clientes antigos.</span></div></button>
      <button class="quiz-btn" type="button" data-answer="correct"><span class="icon-shell">${icon('check-circle')}</span><div><strong>C</strong><br><span>Vender 20 kits em 30 dias para clientes dos últimos seis meses, revisando toda sexta.</span></div></button>
    </div>
...
${slide('slide-13', 'Slide 13: síntese e próximo passo', 'summary tint', `
  <div class="inner">
    <p class="slide-context reveal reveal-up delay-0"><strong>Capítulo 2</strong><span>·</span><span>Fechamento</span></p>
    <header class="summary-header reveal reveal-up delay-1">
      <span class="tag">Síntese</span>
      <h2 class="section-title">O que você deve levar para o <span class="highlight">seu planejamento</span></h2>
    </header>
    <div class="summary-grid stagger-children delay-2">
      <article class="card"><span class="icon-shell">${icon('target')}</span><h3>Meta precisa ser concreta</h3><p>Número, prazo e responsável tornam a intenção executável.</p></article>
      <article class="card"><span class="icon-shell">${icon('clipboard-text')}</span><h3>Plano conecta ação</h3><p>A lista de ações mostra o que acontece nesta semana, não só no fim do trimestre.</p></article>
      <article class="card"><span class="icon-shell">${icon('calendar-check')}</span><h3>Revisão protege rota</h3><p>Você ajusta antes de desperdiçar tempo, caixa ou energia.</p></article>
    </div>
    <div class="takeaway reveal reveal-up delay-4"><span class="icon-shell">${icon('lightbulb')}</span><div><h3>Próximo passo</h3><p>Escreva uma meta de 90 dias e leve para o capítulo 3, onde ela será conectada à organização dos processos.</p></div></div>
  </div>`)}
</main>
<nav class="nav" aria-label="Navegação dos slides">
  ...
  <span class="count" id="count" aria-live="polite">1 / 13</span>
...
console.log(JSON.stringify({ htmlPath, slides: 13 }, null, 2));
